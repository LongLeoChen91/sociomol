...
export const state = {
  ...
  canonicalPresetKey: "custom",
  focusPresetReferenceGeometry: false,
  toggles: {
    showMap: true,
    showModel: true,
    showLandmarks: true,
  },
  modelRepresentation: "cartoon",
  presetKey: "custom",
  presetLandmarks: [],
  arms: [],
  selectedArmId: null,
...
export function setMapContourLevel(value) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric) || numeric <= 0) {
    return;
  }
  state.mapDisplay.contourLevel = numeric;
  notify();
}

export function setCanonicalPresetKey(value) {
  state.canonicalPresetKey = value || "custom";
  notify();
}

export function setFocusPresetReferenceGeometry(value) {
  state.focusPresetReferenceGeometry = Boolean(value);
  notify();
}
