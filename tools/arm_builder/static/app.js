...
import {
  ...
  saveDraftArm,
  selectArm,
  setCanonicalPresetKey,
  setDraftArm,
  setFocusPresetReferenceGeometry,
  setMapContourLevel,
  setMapInfo,
  setModelInfo,
...
const elements = {
  ...
  contourLevel: document.querySelector("#contour-level"),
  modelFile: document.querySelector("#model-file"),
  modelRepresentation: document.querySelector("#model-representation"),
  canonicalPreset: document.querySelector("#canonical-preset"),
  focusPresetReferenceGeometry: document.querySelector("#focus-preset-reference-geometry"),
  generateCanonicalArms: document.querySelector("#generate-canonical-arms"),
  ...
  if (!state.draftArm.point1_xyz || !state.draftArm.point2_xyz) {
    elements.draftSummary.textContent =
      state.capture.active
        ? "Capture mode is active. Click the Anchor first, then the Guide Point, in the 3D viewport."
...
function renderInputs(currentState) {
  ...
  elements.capturePoints.textContent = currentState.capture.active
    ? "Capture in progress..."
    : "Capture Anchor + Guide Point";
  elements.saveArm.textContent = currentState.selectedArmId ? "Update arm" : "Save arm";
  elements.canonicalPreset.value = currentState.canonicalPresetKey;
  const canonicalPreset = getCanonicalModelPresetByKey(currentState.canonicalPresetKey);
  const canFocusPresetReferenceGeometry = Boolean(canonicalPreset?.referenceGeometry?.length);
  elements.focusPresetReferenceGeometry.checked =
    canFocusPresetReferenceGeometry && currentState.focusPresetReferenceGeometry;
  elements.focusPresetReferenceGeometry.disabled = !canFocusPresetReferenceGeometry;
  elements.contourLevel.value = currentState.mapDisplay.contourLevel;
...
function syncViewer(currentState) {
  ...
  viewer.setMapContourLevel(currentState.mapDisplay.contourLevel);
  viewer.setModelVisibility(currentState.toggles.showModel);
  viewer.setPresetReferenceGeometryFocus(
    canonicalPreset,
    currentState.focusPresetReferenceGeometry,
  );
  viewer.setModelRepresentation(currentState.modelRepresentation);
  viewer.renderArms(currentState.arms);
  viewer.renderDraftArm(currentState.draftArm);
...
function commitDraftFromInputsStrict() {
  ...
  if (parsedPoint1.status === "partial" || parsedPoint1.status === "invalid") {
    throw new Error("Anchor coordinate must contain valid X, Y, and Z values.");
  }
  if (parsedPoint2.status === "partial" || parsedPoint2.status === "invalid") {
    throw new Error("Guide Point coordinate must contain valid X, Y, and Z values.");
  }
  setDraftArm(
    elements.armName.value,
...
async function loadMapFromInput() {
  ...
  await viewer.loadMap(file, metadata, state.mapDisplay.contourLevel);
}
...
function generateCanonicalArmsFromModel() {
  if (!state.map.metadata) {
    throw new Error("Load a reference map before generating canonical arms.");
  }
  if (!state.model.fileName) {
    throw new Error("Load a fitted model before generating canonical arms.");
  }

  const preset = getCanonicalModelPresetByKey(state.canonicalPresetKey);
  if (!preset || preset.arms.length === 0) {
    throw new Error("Choose a canonical arm preset before generating arms.");
  }

  const generatedArms = viewer.extractCanonicalArms(preset);
  appendArms(generatedArms);
  return {
    count: generatedArms.length,
    label: preset.label,
  };
}

function populateCanonicalPresetSelect() {
  elements.canonicalPreset.innerHTML = CANONICAL_MODEL_PRESETS.map(
...
function renderValidation(currentState) {
  ...
  if (!expected.point1_xyz || !expected.point2_xyz) {
    elements.validationResult.innerHTML = `
      <div><strong>Current Anchor:</strong> ${formatVector(current.point1_xyz)}</div>
      <div><strong>Current Guide Point:</strong> ${formatVector(current.point2_xyz)}</div>
  ...
  if (!current.point1_xyz || !current.point2_xyz) {
    elements.validationResult.textContent =
      "Current draft arm is incomplete. Fill Anchor / Guide Point coordinates or edit an existing arm.";
    return;
  }
...
elements.contourLevel.addEventListener("input", () => {
  setMapContourLevel(elements.contourLevel.value);
});
...
elements.canonicalPreset.addEventListener("change", (event) => {
  setCanonicalPresetKey(event.target.value);
  const preset = getCanonicalModelPresetByKey(event.target.value);
  if (!preset?.referenceGeometry?.length) {
    setFocusPresetReferenceGeometry(false);
  }
});

elements.focusPresetReferenceGeometry.addEventListener("change", (event) => {
  setFocusPresetReferenceGeometry(event.target.checked);
});

elements.generateCanonicalArms.addEventListener("click", () => {
  try {
    const result = generateCanonicalArmsFromModel();
    setViewerStatus(`Generated ${result.count} arms from ${result.label}.`);
  } catch (error) {
