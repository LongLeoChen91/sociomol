...
plot_f1_vs_l0.py
----------------
Compile and visualize edge-level F1 scores across different ideal linker distances (L0)
...
output_png = os.path.join(script_dir, "f1_vs_l0_comparison.png")
...
def main():
    ...
    if not df_nuc.empty:
        df_nuc = df_nuc.sort_values("L0_NM")
    if not df_ribo.empty:
        df_ribo = df_ribo.sort_values("L0_NM")
    ...
    if not df_nuc.empty and "F1_Score" in df_nuc.columns:
        x_nuc = df_nuc["L0_NM"].to_numpy(dtype=float)
    ...
    if not df_ribo.empty and "F1_Score" in df_ribo.columns:
        x_ribo = df_ribo["L0_NM"].to_numpy(dtype=float)
    ...
    # Labels and title
    # ------------------------------------------------------------
    ax.set_title("SocioMol sensitivity to ideal length (L0)", fontsize=9.5, pad=8)
    ax.set_xlabel("Ideal connection distance L0 (nm)", fontsize=9)
    ...
    x_candidates = []
    if not df_nuc.empty:
        x_candidates.append(float(df_nuc["L0_NM"].max()))
    if not df_ribo.empty:
        x_candidates.append(float(df_ribo["L0_NM"].max()))
    ...
    print(f"\n[OK] F1 vs L0_NM comparison generated at:\n     {output_png}")
