...
# -*- coding: utf-8 -*-
"""
plot_f1_vs_max_half_bending_deg.py
----------------------------------
Compile and visualize edge-level F1 scores across different maximum half bending angles
...
csv_nuc = os.path.join(
    ...
    "experiments",
    "Manual_1",
    "MAX_HALF_BENDING_DEG_sweep_F1.csv"
)
...
output_png = os.path.join(script_dir, "f1_vs_max_half_bending_deg_comparison.png")
...
def main():
    ...
    if not df_nuc.empty:
        df_nuc = df_nuc.sort_values("MAX_HALF_BENDING_DEG")
    if not df_ribo.empty:
        df_ribo = df_ribo.sort_values("MAX_HALF_BENDING_DEG")
    ...
    if not df_nuc.empty and "F1_Score" in df_nuc.columns:
        x_nuc = df_nuc["MAX_HALF_BENDING_DEG"].to_numpy(dtype=float)
    ...
    if not df_ribo.empty and "F1_Score" in df_ribo.columns:
        x_ribo = df_ribo["MAX_HALF_BENDING_DEG"].to_numpy(dtype=float)
    ...
    # Labels and title
    # ------------------------------------------------------------
    ax.set_title("SocioMol sensitivity to max half bending", fontsize=9.5, pad=8)
    ax.set_xlabel("Max half bending angle (deg)", fontsize=9)
    ...
    x_candidates = []
    if not df_nuc.empty:
        x_candidates.append(float(df_nuc["MAX_HALF_BENDING_DEG"].max()))
    if not df_ribo.empty:
        x_candidates.append(float(df_ribo["MAX_HALF_BENDING_DEG"].max()))
    ...
    print(f"\n[OK] F1 vs MAX_HALF_BENDING_DEG comparison generated at:\n     {output_png}")
