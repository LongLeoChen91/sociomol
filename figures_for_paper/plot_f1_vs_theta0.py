...
# -*- coding: utf-8 -*-
"""
plot_f1_vs_theta0.py
--------------------
Compile and visualize edge-level F1 scores across different reference angles (THETA0)
...
output_png = os.path.join(script_dir, "f1_vs_theta0_comparison.png")
...
def main():
    ...
    if not df_nuc.empty:
        df_nuc = df_nuc.sort_values("THETA0_DEG")
    if not df_ribo.empty:
        df_ribo = df_ribo.sort_values("THETA0_DEG")
    ...
    if not df_nuc.empty and "F1_Score" in df_nuc.columns:
        x_nuc = df_nuc["THETA0_DEG"].to_numpy(dtype=float)
    ...
    if not df_ribo.empty and "F1_Score" in df_ribo.columns:
        x_ribo = df_ribo["THETA0_DEG"].to_numpy(dtype=float)
    ...
    # Labels and title
    # ------------------------------------------------------------
    ax.set_title("SocioMol sensitivity to ref angle (THETA0)", fontsize=9.5, pad=8)
    ax.set_xlabel("Reference angle tolerance THETA0 (deg)", fontsize=9)
    ...
    x_candidates = []
    if not df_nuc.empty:
        x_candidates.append(float(df_nuc["THETA0_DEG"].max()))
    if not df_ribo.empty:
        x_candidates.append(float(df_ribo["THETA0_DEG"].max()))
    ...
    print(f"\n[OK] F1 vs THETA0_DEG comparison generated at:\n     {output_png}")
